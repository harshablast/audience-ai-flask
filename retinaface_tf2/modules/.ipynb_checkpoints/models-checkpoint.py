...
def RetinaFaceModel(cfg, training=False, iou_th=0.4, score_th=0.02,
    ...
    features = [SSH(out_ch=out_ch, wd=wd, name='SSH_' + str(i))(f)
                for i, f in enumerate(fpn)]

    bbox_regressions = tf.concat(
        [BboxHead(num_anchor, wd=wd, name='BboxHead_' + str(i))(f)
         for i, f in enumerate(features)], axis=1)
    landm_regressions = tf.concat(
        [LandmarkHead(num_anchor, wd=wd, name='LandmarkHead_' + str(i))(f)
         for i, f in enumerate(features)], axis=1)
    classifications = tf.concat(
        [ClassHead(num_anchor, wd=wd, name='ClassHead_' + str(i))(f)
         for i, f in enumerate(features)], axis=1)
